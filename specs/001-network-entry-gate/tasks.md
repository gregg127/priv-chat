...
- [X] T003 [P] Initialise `api-gateway` Gradle project (Spring Boot 4.0.4) at `implementation/api-gateway/build.gradle` with dependencies: `spring-boot-starter-web`, `spring-boot-starter-actuator`. Routing is handled by `AuthProxyController` (RestClient-based reverse proxy) rather than Spring Cloud Gateway, which had incompatible auto-configuration with Spring Boot 4.x.
...
- [X] T011 [P] Configure Spring Session JDBC in `implementation/services/entry-auth-service/src/main/java/com/privchat/auth/config/SessionConfig.java`: `@EnableJdbcHttpSession`, `DefaultCookieSerializer` with `HttpOnly=true`, `Secure` driven by `SESSION_COOKIE_SECURE` env var (default `false`), `SameSite=Strict`, `CookiePath=/`; session timeout applied via `@PostConstruct` on `JdbcIndexedSessionRepository` using `SESSION_TIMEOUT_SECONDS` env var
